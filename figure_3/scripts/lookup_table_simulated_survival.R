...
survival_params <- survival_params %>% sample_n(1e3)
...
uncensored_data_unique <- uncensored_data %>% 
  distinct(across(c(growth_rate, drift, growth_decay, immuno_effect)))
...
simulate <- function(df){
  future_pmap(list(R = df$growth_rate, 
                   ...
                   raise_killing = df$treatment_effect),
...
save(df, file = "data/lookup_table_simulated_survival.RData")
