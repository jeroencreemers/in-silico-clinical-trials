...
load("data/figure_4d_data.RData")
...
ggplot(df, aes(x = arm_size, y = power, lty = ID)) +
  ...
  scale_x_continuous(name = "Patients per arm (n)", breaks = seq(500, 3000, 500)) +
...
ggsave(filename = "plots/figure_4D.pdf", plot = last_plot(), width = 6.5, height = 5, units = "cm", useDingbats = FALSE)