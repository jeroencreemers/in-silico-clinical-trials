### Figure 3E Panel 2 Plot ###
library(ggplot2)

source("scripts/theme.R")

load("data/figure_3e_data_panel_2.RData")

figure_3e_panel_2 <- ggplot(figure_3e_data_panel_2, aes(x = time, group=arm)) +
  geom_ribbon(aes(ymin = ci_low, ymax = ci_high, group = arm), alpha = 0.2) +
  geom_line(aes(y = hazard, col = arm)) +
  scale_color_manual(values = c("red", "#64A5B1")) +
...
save(figure_3e_panel_2, file = "data/figure_3e_panel_2.RData")