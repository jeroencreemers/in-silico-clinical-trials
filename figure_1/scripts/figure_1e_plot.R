...
source("../../misc/theme.R")
load("data/figure_1e_data.RData")
...
res$table <- res$table + 
  ...
  scale_y_discrete(labels = c("DTIC", "Ipi+DTIC"), limits = rev)
...
ggsave(filename = "plots/figure_1e.pdf", plot = print(res, newpage = FALSE), width = 6, height = 5, units = "cm", useDingbats = FALSE)