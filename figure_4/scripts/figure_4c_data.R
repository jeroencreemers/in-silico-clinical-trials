...
################################################################################
# Chemoimmunotherapy vs. Chemotherapy -> Log-rank test #
...
power_chemoimmuno_logrank <- rearrange_output(output)
################################################################################
# Chemoimmunotherapy vs. Chemotherapy -> Pearson's Chi-squared test #
...
power_chemoimmuno_proptest <- rearrange_output(output)
################################################################################

df <- bind_rows(list(logrank = power_chemoimmuno_logrank, proptest = power_chemoimmuno_proptest), .id="ID")

################################################################################

save(df, file = "data/figure_4c_data.RData")
