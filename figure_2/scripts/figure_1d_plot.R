...
fit<- survfit( Surv(OS, status)~1, data = simulated_survival_data)

res <- ggsurvplot(fit, data = simulated_survival_data, palette = "black", 
                  break.x.by = 4*30.44,
...
ggsave(filename = "plots/figure_1d.pdf", plot = print(res, newpage = FALSE), width = 6, height = 5, units = "cm", useDingbats = FALSE)
