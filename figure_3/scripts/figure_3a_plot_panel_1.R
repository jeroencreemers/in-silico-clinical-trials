### Figure 3A Panel 1 Plot ###
library(ggplot2)
library(survival)

source("scripts/theme.R")

load("data/figure_3a_data_panel_1.RData")

fit <- survfit(Surv(OS, status)~randomization, data = simulated_data)

figure_3a_panel_1 <- survminer::ggsurvplot(fit = fit, palette = c("grey", "black"), 
  ...
  geom_segment(x = 40, y = 0.11, xend = 6*30.4, yend = 0.11, size = 0.4, lineend = "round")

save(figure_3a_panel_1, file = "data/figure_3a_panel_1.RData")