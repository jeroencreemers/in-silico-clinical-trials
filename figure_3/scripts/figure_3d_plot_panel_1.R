### Figure 3D Panel 1 Plot ###
library(ggplot2)
library(survival)

source("scripts/theme.R")

load("data/figure_3d_data_panel_1.RData")

fit <- survfit(Surv(OS, status)~therapy, data = simulated_data)

figure_3d_panel_1 <- survminer::ggsurvplot(fit = fit, palette = c("black", "red"), 
  ...
  annotate("text", x = 0, y = c(0.02, 0.12), label = c("C", "T"), size = 2, col = c("black", "red")) +
...
save(figure_3d_panel_1, file = "data/figure_3d_panel_1.RData")