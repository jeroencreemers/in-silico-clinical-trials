...
load("data/figure_3c_dataset.RData")
################################################################################################
### Figure 3C: Hazard plot data (panel 2) ###
################################################################################################
...
fit_treatment <- bshazard(Surv(OS, status)~1, data = df[df$therapy=="chemoimmuno", ], nbin = 48, lambda = 10000, alpha = 1)
...
figure_3c_data_panel_2 <- pivot_longer(hazards_df, -time, names_sep = "_arm_", names_to = c(".value", "arm"))

save(figure_3c_data_panel_2, file = "data/figure_3c_data_panel_2.RData")
